...
When the robot tells a joke, it needs to make adaptive transitions that correlates with the response. For example, if a joke does well and is received with laughter, the robot needs to time the next joke so that it is delivering the content when the audience can hear the joke. However, if a joke is not well received by the audience, the time a robot needs to wait for the next joke will be different than if there is laughter. This is important for the effectiveness of a performance, as the connotation of the next joke is determined by the result of the next joke; an audience that is told a bad joke will be hesitant to enjoy a joke if the previous jokes were bad. A robot comedian needs to be able to stay in a joke if the audience likes it, or address/recover the bad joke before starting a new sequence.
...
To test the effectiveness of our robot comedian, we will conduct tests on the three research questions.
We will create scripts with varying degrees of (1) crowdwork, (2) character, and (3) adaptiveness.
We hypothesize that the scripts with greater degrees of crowdwork, character, and adaptiveness, will create more effective comedy, and have a more positive response from the audience.
...
Likewise, testing the effectiveness of adaptivity will include scripts with no adaptivity and scripts that adjust to audience response.
The audio sensors on the NAO bot will be used to evaluate audience reception to a joke, which the robot can then adapt to.
For instance, if one joke performs very well, the next joke from the robot might be a similar joke.
% Something about adaptive algorithms here?
These scripts will be run on the NAO robot, and performed in front of a small audience in the testing phase, and later a larger live audience in the evaluation phase.
