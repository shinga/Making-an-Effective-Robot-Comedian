...
The audio sensors on the NAO bot will be used to evaluate audience reception to a joke, which the robot can then adapt to. The microphone on the NAO does not perform well when receiving input from a large audience, and is designed to handle smaller scale interactions. NAO interprets words with the ALSpeechRecognitionProxy and ALTextToSpeechProxy. 
% TODO put these in the glossary, and add more about microphones
